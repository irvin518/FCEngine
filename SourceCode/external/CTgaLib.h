...
public:
    ...
    static bool TgaLoadHeader(unsigned char *buffer, unsigned long bufSize, SImageTGA *info);

    /// loads the image pixels. You shouldn't call this function directly
    static bool TgaLoadImageData(unsigned char *buffer, unsigned long bufSize, SImageTGA *info);

    /// this is the function to call when we want to load an image buffer.
    static SImageTGA* TgaLoadBuffer(unsigned char* buffer, long size);

    /// this is the function to call when we want to load an image
    static SImageTGA * TgaLoad(const TCHAR *filename);
