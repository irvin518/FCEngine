...
bool CTgaLib::TgaLoadHeader( unsigned char *buffer, unsigned long bufSize, SImageTGA *info )
{
    bool ret = false;
...
bool CTgaLib::TgaLoadImageData( unsigned char *buffer, unsigned long bufSize, SImageTGA *info )
{
    bool ret = false;
    do 
    {
...
SImageTGA* CTgaLib::TgaLoadBuffer( unsigned char* buffer, long size )
{
    int mode,total;
...
bool CTgaLib::TgaLoadRLEImageData( unsigned char* buffer, unsigned long bufSize, TImageTGA *info )
{
...
SImageTGA * CTgaLib::TgaLoad( const TCHAR *filename )
{
    SImageTGA * pReturn = NULL;
    CData data = CFileUtils::GetInstance()->GetDataFromFile(filename);

    if (!data.IsNull())
    {
        pReturn = TgaLoadBuffer(data.GetBytes(), data.GetSize());
    }
