...
void CS3tc::S3tc_decode( uint8_t *encodeData, uint8_t *decodeData, const int pixelsWidth, const int pixelsHeight, ES3TCDecodeFlag decodeFlag )
{
    ...
    for (int block_y = 0; block_y < pixelsHeight / 4; ++block_y, decodeBlockData += 3 * pixelsWidth)   //stride = 3*width
    {
        for(int block_x = 0; block_x < pixelsWidth / 4; ++block_x, decodeBlockData += 4)            //skip 4 pixels
        {
            ...
            switch (decodeFlag)
            {
            case ES3TCDecodeFlag::DXT1:
                {
                    S3tc_decode_block(&encodeData, decodeBlockData, pixelsWidth, 0, 0LL, ES3TCDecodeFlag::DXT1);
                }
                break;
            case ES3TCDecodeFlag::DXT3:
                {
                    memcpy((void *)&blockAlpha, encodeData, 8);
                    encodeData += 8;
                    S3tc_decode_block(&encodeData, decodeBlockData, pixelsWidth, 1, blockAlpha, ES3TCDecodeFlag::DXT3);
                }
                break;
            case ES3TCDecodeFlag::DXT5:
                {
                    memcpy((void *)&blockAlpha, encodeData, 8);
                    encodeData += 8;
                    S3tc_decode_block(&encodeData, decodeBlockData, pixelsWidth, 1, blockAlpha, ES3TCDecodeFlag::DXT5);
                }
                break;
            default:
                break;
...
void CS3tc::S3tc_decode_block(uint8_t **blockData, uint32_t *decodeBlockData, unsigned int stride, bool oneBitAlphaFlag, uint64_t alpha, ES3TCDecodeFlag decodeFlag)
{
    ...
    if (ES3TCDecodeFlag::DXT5 == decodeFlag)
    {
