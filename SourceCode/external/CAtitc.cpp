...
void CAtitc::Atitc_decode( uint8_t *encodeData, uint8_t *decodeData, const int pixelsWidth, const int pixelsHeight, EATITCDecodeFlag decodeFlag )
{
    ...
    for (int block_y = 0; block_y < pixelsHeight / 4; ++block_y, decodeBlockData += 3 * pixelsWidth)   //stride = 3*width
    {
        for (int block_x = 0; block_x < pixelsWidth / 4; ++block_x, decodeBlockData += 4)            //skip 4 pixels
        {
            ...
            switch (decodeFlag)
            {
            case EATITCDecodeFlag::ATC_RGB:
                {
                    Atitc_decode_block(&encodeData, decodeBlockData, pixelsWidth, 0, 0LL, EATITCDecodeFlag::ATC_RGB);
                }
                break;
            case EATITCDecodeFlag::ATC_EXPLICIT_ALPHA:
                {
                    memcpy((void *)&blockAlpha, encodeData, 8);
                    encodeData += 8;
                    Atitc_decode_block(&encodeData, decodeBlockData, pixelsWidth, 1, blockAlpha, EATITCDecodeFlag::ATC_EXPLICIT_ALPHA);
                }
                break;
            case EATITCDecodeFlag::ATC_INTERPOLATED_ALPHA:
                {
                    memcpy((void *)&blockAlpha, encodeData, 8);
                    encodeData += 8;
                    Atitc_decode_block(&encodeData, decodeBlockData, pixelsWidth, 1, blockAlpha, EATITCDecodeFlag::ATC_INTERPOLATED_ALPHA);
                }
                break;
            default:
                break;
...
void CAtitc::Atitc_decode_block( uint8_t **blockData, uint32_t *decodeBlockData, unsigned int stride, bool oneBitAlphaFlag, uint64_t alpha, EATITCDecodeFlag decodeFlag )
{
    ...
    if (EATITCDecodeFlag::ATC_INTERPOLATED_ALPHA == decodeFlag)
    {
