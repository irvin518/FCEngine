...
#include "external/CZipUtils.h"
#include "external/CS3tc.h"
#include "jpeglib.h"
#include "png.h"
#include "tiffio.h"
#include "decode.h"
#include "external/CTgaLib.h"
#include "external/CAtitc.h"
...
namespace
{
    ...
    {
        FC_UNUSED_PARAM(fd);
        FC_UNUSED_PARAM(pbase);
        FC_UNUSED_PARAM(psize);
        return 0;
    }
...
#pragma pack(pop)
    ...
    {
        bool bReturn = true;
        if (!CConfingation::GetInstance()->SupportsPVRTC())
        {
...
{
    bool ret = false;
    m_filePath = CFileUtils::GetInstance()->FullPathForFilename(path);
    ...
    for(int i = 0; i < nrPixels; i++)
    {
        ...
        tmp[i] = FC_RGB_PREMULTIPLY_ALPHA( p[0], p[1], p[2], p[3] );
    }

    SDL_FreeSurface(iSurf);
#else
    CData data = CFileUtils::GetInstance()->GetDataFromFile(m_filePath);
...
bool CImage::InitWithJpgData( const unsigned char * data, ssize_t dataLen )
{
    ...
    do 
    {
        ...
        while( cinfo.output_scanline < cinfo.output_height )
        {
            ...
            {
                m_data[location++] = row_pointer[0][i];
            }
        }
...
#if (FC_PLATFOEM != PLATFORM_BADA && FC_PLATFOEM != PLATFORM_NACL)
...
#if (FC_PLATFOEM == PLATFORM_WP8) || (FC_PLATFOEM == PLATFORM_WINRT)
...
bool CImage::InitWithPVRv2Data( const unsigned char * data, ssize_t dataLen )
{
    ...
    do 
    {
        ...
        CConfingation *configuration = CConfingation::GetInstance();

        m_hasPremultipliedAlpha = false;
        unsigned int flags = FC_SWAP_INT32_LITTLE_TO_HOST(header->flags);
        ...
        //Get size of mipmap
        m_width = width = FC_SWAP_INT32_LITTLE_TO_HOST(header->width);
        m_height = height = FC_SWAP_INT32_LITTLE_TO_HOST(header->height);

        //Get ptr to where data starts..
        dataLength = FC_SWAP_INT32_LITTLE_TO_HOST(header->dataLength);
        ...
        while (dataOffset < dataLength)
        {
            ...
            case PVR2TexturePixelFormat::BGRA8888:
                if (CConfingation::GetInstance()->SupportsBGRA8888() == false)
                {
...
bool CImage::InitWithPVRv3Data( const unsigned char * data, ssize_t dataLen )
{
    ...
    do 
    {
        ...
        // validate version
        if (FC_SWAP_INT32_BIG_TO_HOST(header->version) != 0x50565203)
        {
        ...
        // flags
        int flags = FC_SWAP_INT32_LITTLE_TO_HOST(header->flags);
        ...
        // sizing
        int width = FC_SWAP_INT32_LITTLE_TO_HOST(header->width);
        int height = FC_SWAP_INT32_LITTLE_TO_HOST(header->height);
        m_width = width;
        m_height = height;
        ...
        for (int i = 0; i < m_numberOfMipmaps; i++)
        {
            ...
            case PVR3TexturePixelFormat::BGRA8888:
                if( ! CConfingation::GetInstance()->SupportsBGRA8888())
                {
                    BEATS_PRINT(_T("image. BGRA8888 not supported on this device"));
                    break;
                }
...
bool CImage::InitWithETCData( const unsigned char * data, ssize_t dataLen )
{
    ...
    do 
    {
        ...
        if(CConfingation::GetInstance()->SupportsETC())
        {
...
bool CImage::InitWithS3TCData( const unsigned char * data, ssize_t dataLen )
{
    ...
    if (CConfingation::GetInstance()->SupportsS3TC())  //compressed data length
    {
    ...
    if (CConfingation::GetInstance()->SupportsS3TC())
...
bool CImage::InitWithATITCData( const unsigned char *data, ssize_t dataLen )
{
    ...
    if (CConfingation::GetInstance()->SupportsATITC())  //compressed data length
    {
    ...
    for (int i = 0; i < m_numberOfMipmaps && (width || height); ++i)
    {
        ...
        if (CConfingation::GetInstance()->SupportsATITC())
        {
            /* decode texture throught hardware */
...
bool CImage::SaveImageToPNG( const TString& filePath, bool isToRGB /*= true*/ )
{
    bool bRet = false;
    do 
    {
        ...
        char path[ MAX_PATH ];
        CStringHelper::GetInstance()->ConvertToCHAR( filePath.c_str(), path, filePath.length() );
        fp = fopen(path, "wb");
        if(NULL == fp)
        {
            break;
        ...
        if (!HasAlpha())
        {
            ...
            {
                row_pointers[i] = (png_bytep)m_data + i * m_width * 3;
            }
        ...
        else
        {
            if (isToRGB)
            {
                ...
                for (int i = 0; i < (int)m_height; i++)
                {
                    row_pointers[i] = (png_bytep)pTempData + i * m_width * 3;
                }
...
bool CImage::InitWithImageFileThreadSafe( const TString& fullpath )
{
    ...
    CData data = CFileUtils::GetInstance()->GetDataFromFile(fullpath);
...
bool CImage::IsPvr( const unsigned char * data, ssize_t dataLen )
{
    ...
    if (static_cast<size_t>(dataLen) > sizeof(PVRv2TexHeader) && static_cast<size_t>(dataLen) > sizeof(PVRv3TexHeader))
    {
        ...
        ret = memcmp(&headerv2->pvrTag, gPVRTexIdentifier, strlen(gPVRTexIdentifier)) == 0 || FC_SWAP_INT32_BIG_TO_HOST(headerv3->version) == 0x50565203;
    }
    return ret;
}
...
bool CImage::IsS3TC( const unsigned char * data,ssize_t dataLen )
{
    ...
    {
        BEATS_PRINT(_T("the file is not a dds file!"));
    }
    return ret;
}
...
bool CImage::IsATITC( const unsigned char *data, ssize_t dataLen )
{
    ...
    {
        BEATS_PRINT(_T("the file is not a ktx file!"));
    }
    return ret;
}
