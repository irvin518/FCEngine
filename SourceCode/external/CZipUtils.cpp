...
{
    bool bReturn = false;
    CData compressedData = CFileUtils::GetInstance()->GetDataFromFile(filePath);

    if (!compressedData.IsNull())
    {
        bReturn = IsGZipBuffer(compressedData.GetBytes(), compressedData.GetSize());
    }
    return bReturn;
}
...
int CZipUtils::InflateCCZFile( const TCHAR *filePath, unsigned char **out )
{
    ...
    if (compressedData.IsNull())
    {
        BEATS_PRINT(_T("Error loading CCZ compressed file"));
    }
    else
    {
        returnInt = InflateCCZBuffer(compressedData.GetBytes(), compressedData.GetSize(), out);
    }
    return returnInt;
}
...
int CZipUtils::InflateCCZBuffer( const unsigned char *buffer, ssize_t bufferLen, unsigned char **out )
{
    ...
    do
    {
        ...
        if( header->sig[0] == 'C' && header->sig[1] == 'C' && header->sig[2] == 'Z' && header->sig[3] == '!' )
        {
            ...
            unsigned int version = FC_SWAP_INT16_BIG_TO_HOST( header->version );
            if( version > 2 )
            {
            ...
            if( FC_SWAP_INT16_BIG_TO_HOST(header->compression_type) != CCZ_COMPRESSION_ZLIB )
            {
        ...
        else if( header->sig[0] == 'C' && header->sig[1] == 'C' && header->sig[2] == 'Z' && header->sig[3] == 'p' )
        {
            ...
            unsigned int required = FC_SWAP_INT32_BIG_TO_HOST( header->reserved );
        ...
        unsigned int len = FC_SWAP_INT32_BIG_TO_HOST( header->len );
...
bool CZipUtils::IsCCZFile( const TCHAR *path )
{
    bool bReturn = false;
    // load file into memory
    CData compressedData = CFileUtils::GetInstance()->GetDataFromFile(path);

    if (compressedData.IsNull())
    {
        BEATS_PRINT(_T("ZipUtils: loading file failed"));
    }
    else
    {
        bReturn = IsCCZBuffer(compressedData.GetBytes(), compressedData.GetSize());
    }
    return bReturn;
}
