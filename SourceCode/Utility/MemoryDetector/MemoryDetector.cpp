...
#ifdef _DEBUG
    m_processInDebugMode = true;
#endif
#ifdef _UNICODE
    m_processInUnicodeMode = true;
#endif
    size_t curEip;
...
bool CMemoryDetector::GetHookModuleName( char*& crtLibName, char*& mfcLibName )
{
    if (m_processInDebugMode)
    {
        mfcLibName = m_processInUnicodeMode ? "mfc110ud.dll" : "mfc110d.dll";
        crtLibName = "msvcr110d.dll";
    }
    else
    {
        mfcLibName = m_processInUnicodeMode ? "mfc110u.dll" : "mfc110.dll";
        crtLibName = "msvcr110.dll";
    }
    return true;
}
