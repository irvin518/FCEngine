...
{
    FC_PERFORMDETECT_START(ePNT_RenderText1)
    CQuadC quadC;
    quadC.tl = color;
    quadC.tr = color;
    quadC.bl = color;
    quadC.br = color;
    FC_PERFORMDETECT_STOP(ePNT_RenderText1)

    FC_PERFORMDETECT_START(ePNT_RenderText2)
    auto glyphs = GetGlyphs(text);
    FC_PERFORMDETECT_STOP(ePNT_RenderText2)
    FC_PERFORMDETECT_START(ePNT_RenderText3)
    for(auto glyph : glyphs)
    {
        DrawGlyph(glyph, x, y, quadC, transform);
        x += glyph->width;
    }
    FC_PERFORMDETECT_STOP(ePNT_RenderText3)
}

void CFontFace::DrawGlyph(CFontGlyph *glyph, kmScalar x, kmScalar y, 
                         const CQuadC &colorRect, const kmMat4 *transform) const
{
    FC_PERFORMDETECT_START(ePNT_RenderText4)
    CQuadPTC quad;
    ...
    FC_PERFORMDETECT_STOP(ePNT_RenderText4)

    FC_PERFORMDETECT_START(ePNT_RenderText5)
    auto itr = m_materialMap.find(glyph->texture->ID());
    BEATS_ASSERT(itr != m_materialMap.end());
    FC_PERFORMDETECT_STOP(ePNT_RenderText5)

    FC_PERFORMDETECT_START(ePNT_RenderText6)
    CRenderBatch *batch = CFontManager::GetInstance()->GetRenderGroup()->GetRenderBatch(
        itr->second, GL_TRIANGLES, true);
    FC_PERFORMDETECT_STOP(ePNT_RenderText6)
    FC_PERFORMDETECT_START(ePNT_RenderText7)
    batch->AddQuad(quad);
    FC_PERFORMDETECT_STOP(ePNT_RenderText7)
}
