...
CWindow::~CWindow()
{
    CWindowManager::GetInstance()->UnregisterWindow(this);
...
void CWindow::ReflectData(CSerializer& serializer)
{
    ...
    DECLARE_PROPERTY(serializer, m_pRenderer, true, 0xFFFFFFFF, _T("渲染器"), NULL, NULL, NULL);
    DECLARE_DEPENDENCY_LIST(serializer, m_children, _T("子窗口"), eDT_Weak);
}

bool CWindow::OnPropertyChange(void* pVariableAddr, void* pNewValueToBeSet)
{
    ...
    if (!bHandled)
    {
        ...
        {
            TString strName;
            DeserializeVarialble(strName, (CSerializer*) pNewValueToBeSet);
            SetName(strName);
            bHandled = true;
        }
        else if(pVariableAddr == &m_pos)
        {
            kmVec2 data;
            DeserializeVarialble(data, (CSerializer*) pNewValueToBeSet);
            SetPos(data);
            bHandled = true;
        }
        else if(pVariableAddr == &m_percentPos)
        {
            kmVec2 data;
            DeserializeVarialble(data, (CSerializer*) pNewValueToBeSet);
            SetPercentPos(data);
        ...
        else if(pVariableAddr == &m_percentSize)
        {
            kmVec2 data;
            DeserializeVarialble(data, (CSerializer*) pNewValueToBeSet);
            SetPercentSize(data);
        ...
        else if(pVariableAddr == &m_fRotation)
        {
            kmScalar data;
            DeserializeVarialble(data, (CSerializer*) pNewValueToBeSet);
            SetRotation(data);
        ...
        else if(pVariableAddr == &m_bVisible)
        {
            bool data;
            DeserializeVarialble(data, (CSerializer*) pNewValueToBeSet);
            SetVisible(data);
...
void CWindow::SetName(const TString &name)
{
    if(name != m_strName)
    {
        ...
        m_strName = name;
        if(!Parent() && CWindowManager::GetInstance()->RootWindow())
        {
            CWindowManager::GetInstance()->RootWindow()->AddChild(this);
        }
    }
}
