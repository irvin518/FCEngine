...
void CBaseRenderer::ReflectData(CSerializer& serializer)
{
    super::ReflectData(serializer);
    DECLARE_PROPERTY(serializer, m_stateLayers, true, 0xFFFFFFFF, _T("äÖÈ¾²ã"), NULL, NULL, NULL);
}
...
void CBaseRenderer::AddLayer(const TString &textureFragName, unsigned int state)
{
    ...
    m_stateLayers[state].push_back(new CRenderLayer(atlasName, fragName));
}
