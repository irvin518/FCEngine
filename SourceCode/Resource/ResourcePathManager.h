...
public:
    ...
    void Init();
    const TString& GetResourcePath(CResourcePathManager::EResourcePathType type);
    CResourcePathManager::EResourcePathType GetResourcePathType(EResourceType type) const;
    const TString& GetFullPathForFilename( const TString &filename );

private:
    TString m_strResourcePath;
    TString m_strFullPathForFilename;
};
#endif