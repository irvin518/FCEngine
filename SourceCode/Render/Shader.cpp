...
bool CShader::Load()
{
    const TCHAR* pszExtension = PathFindExtension(GetFilePath().c_str());
    bool bIsVS = _tcsicmp(pszExtension, _T(".vs")) == 0;
    m_shaderType = bIsVS ? GL_VERTEX_SHADER : GL_FRAGMENT_SHADER;
    bool bRet = Compile(&m_uId, GetFilePath().c_str());
...
#endif
        TString strPath = CResourcePathManager::GetInstance()->GetResourcePath(CResourcePathManager::eRPT_Shader);
