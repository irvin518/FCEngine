...
void CAnimationController::Update(float fDeltaTime)
{
    if(m_bPlaying && m_pCurrentAnimation && m_pSkeleton)
    {
        ...
        FC_PERFORMDETECT_START(ePNT_AnimationManager_CalcMatrix)
        CalcDeltaMatrices();
        FC_PERFORMDETECT_STOP(ePNT_AnimationManager_CalcMatrix)
    }
}
...
void CAnimationController::CalcDeltaMatrices()
{
    ...
    if(CheckBlend())
    {
        FC_PERFORMDETECT_START(ePNT_AnimationManager_Blend)
        BlendAnimation(m_pPreAnimation,m_pCurrentAnimation);
        FC_PERFORMDETECT_STOP(ePNT_AnimationManager_Blend)
    }

    const std::vector<ESkeletonBoneType>& bones = m_pCurrentAnimation->GetBones();

    FC_PERFORMDETECT_START(ePNT_AnimationManager_DeltaMatrix)
    BoneMatrix bonesMat;
    ...
    FC_PERFORMDETECT_STOP(ePNT_AnimationManager_DeltaMatrix)
}
