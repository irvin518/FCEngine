...
bool CRenderManager::Initialize()
{
    ...
    if (bRet)
    {
        ...
        CConfingation *conf = CConfingation::GetInstance();
        conf->GatherGPUInfo();
        BEATS_PRINT(_T("%s\n"),conf->GetInfo().c_str());
    ...
    m_pLineMaterial->GetRenderState()->SetLineWidth(2.f);
    m_pLineMaterial->GetRenderState()->SetPointSize(5.f);
    m_pLineMaterial->SetTexture(0, nullptr);

    m_p2DRenderGroup = static_cast<CBatchRenderGroup *>(
...
void CRenderManager::Render()
{
    if (m_pCamera)
    {
        ...
        FC_PERFORMDETECT_START(ePNT_ParticleRender)
        //particle system
        FCEngine::CParticleSystemManager::GetInstance()->Render();
        FC_PERFORMDETECT_STOP(ePNT_ParticleRender)
...
void CRenderManager::Render2DLine(const CVertex2DPC& start, const CVertex2DPC& end)
{
    ...
    m_p2DRenderGroup->GetRenderBatch(m_pLineMaterial, GL_LINES, false)->AddVertices(vertices, 2);
}
...
void CRenderManager::Render2DTriangle(const CVertex2DPC& pt1, const CVertex2DPC& pt2, const CVertex2DPC& pt3)
{
    ...
    m_p2DRenderGroup->GetRenderBatch(m_pLineMaterial, GL_TRIANGLES, false)->AddVertices(vertices, 3);
}
                                 ...
                                 bool bRenderOutline, CColor outlineColor)
{
    ...
    CRenderBatch *batch = m_p2DRenderGroup->GetRenderBatch(m_pLineMaterial, 
        isHalfFan ? GL_TRIANGLE_STRIP : GL_TRIANGLE_FAN, false);
    CRenderBatch *outlineBatch = nullptr;
    if(bRenderOutline)
    {
        outlineBatch = m_p2DRenderGroup->GetRenderBatch(m_pLineMaterial, GL_LINE_LOOP, false);
    }

    if(!isHalfFan)
    {
        CVertexPTC verOrigin;
