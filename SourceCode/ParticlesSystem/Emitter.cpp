...
namespace FCEngine
{
    ...
    bool CParticleEmitter::OnPropertyChange(void* pVariableAddr, void* pNewValueToBeSet)
    {
        ...
        if (!bRet)
        {
            ...
            {
                DeserializeVarialble(m_uTotalCount, (CSerializer*)pNewValueToBeSet);
                InitVAOAndVBO( m_uTotalCount );
                bRet = true;
            }
            else if( &m_uRatePerSecond == pVariableAddr )
            {
                DeserializeVarialble(m_uRatePerSecond, (CSerializer*)pNewValueToBeSet);
                SetShootVelocity( m_uRatePerSecond );
                bRet = true;
            }
        }
