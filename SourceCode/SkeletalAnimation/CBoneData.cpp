...
const std::vector<SDisplayData>& CBoneData::GetDisplayDataList() const
{
    return m_displayDataList;
}

void CBoneData::AddDisplayData(const SDisplayData& data)
{
    m_displayDataList.push_back(data);
}

SDisplayData CBoneData::ReplaceDisplayData(const SDisplayData& displayData, size_t index /*= 0*/)
{
    BEATS_ASSERT(index < m_displayDataList.size());
    SDisplayData displaydata = m_displayDataList[index];
