...
EVT_TREE_ITEM_ACTIVATED(Ctrl_Tree_Resource, CEditorMainFrame::OnActivateComponentInstanceTreeItem)
...
void CEditorMainFrame::CreateAuiNoteBook()
{
    ...
    client_size = wxSize(client_size.GetWidth() * PANELSIZESCALE, client_size.GetHeight() * PANELSIZESCALE);
    ...
    pSizer = new wxBoxSizer(wxVERTICAL);
    m_pResourcePanel = new wxPanel(this, wxID_ANY, wxDefaultPosition, client_size);
...
void CEditorMainFrame::CreatePropertyGrid()
{
    ...
    m_pPropGridManager->Create(m_pRight, wxID_ANY, wxDefaultPosition, wxDefaultSize, style );
...
void CEditorMainFrame::CreateTreeCtrl()
{
    ...
    m_pComponentFileTC = new wxTreeCtrl(m_pLeft, Ctrl_Tree_CompontentFile, wxPoint(0,0), wxDefaultSize, iStyle);
    ...
    m_pComponentInstanceTreeCtrl = new wxTreeCtrl(m_pLeft, Ctrl_Tree_CompontentInstance, wxPoint(0,0), wxDefaultSize, iStyle);
    ...
    m_pComponentTC = new wxTreeCtrl(m_pResourceBook, Ctrl_Tree_Compontent, wxPoint(0,0), wxDefaultSize, iStyle);
    ...
    m_pResourceTreeCtrl = new wxTreeCtrl(m_pResourceBook, Ctrl_Tree_Resource, wxPoint(0,0), wxSize(160,250), iStyle);
...
void CEditorMainFrame::AddPageToBook()
{
    ...
    m_pResourceBook->AddPage( m_pComponentTC, wxT("Components") );
...
void CEditorMainFrame::GetPerformanceDialog()
{
    ...
    {
        m_pWPerformance = new EditPerformanceDialog(this, wxID_ANY, wxT("Performance"), wxPoint(500,200), wxSize(1000,700), wxDEFAULT_DIALOG_STYLE ^ wxMINIMIZE_BOX);
    }
    if (m_bIsLanguageSwitched)
    {
...
void CEditorMainFrame::GetMemoryDialog()
{
    if (m_pWMemory == NULL)
    {
        m_pWMemory = new EditMemoryDialog(this, wxID_ANY, wxT("Memory"), wxPoint(500,200), wxSize(850,700), wxDEFAULT_DIALOG_STYLE ^ wxMINIMIZE_BOX);
    }
    if (m_bIsLanguageSwitched)
    {
        m_pWMemory->LanguageSwitch();
    }
    m_pWMemory->wxDialog::Show();
}
...
void CEditorMainFrame::OnAuiButton(wxCommandEvent& event)
{
    ...
    case ID_TB_FlushProperty:
        if (m_Manager.GetPane(m_pBottomPanel).IsShown())
        {
            m_Manager.GetPane(m_pBottomPanel).Hide();
        }
        else
        {
            m_Manager.GetPane(m_pBottomPanel).Show();
        }
        m_Manager.Update();
        break;
...
void CEditorMainFrame::OnActivateComponentFile( wxTreeEvent& event )
{
    ...
    else if (_tcsicmp(pData->GetFileName().c_str(), curWorkingFile.c_str()) != 0)
    {
        ...
        if (m_activeItemId != itemId)
        {
            if (m_activeItemId)
            {
                m_pComponentFileTC->SetItemBackgroundColour(m_activeItemId, m_itemTextcolor);
            }
            m_pComponentFileTC->SetItemBackgroundColour(itemId, wxColor(150, 33, 184));
            m_pCenter->SetPageText(0, m_pComponentFileTC->GetItemText(itemId));
        }
...
void CEditorMainFrame::ChangeLanguage()
{
    ...
    m_pToolBarPerform->SetToolLabel(ID_TB_MemoryBtn, pLanguageManager->GetText(eL_Memory));
...
EditMemoryDialog* CEditorMainFrame::GetMemoryDialogPtr()
{
    return m_pWMemory;
}
...
void CEditorMainFrame::ShowMemoryDlg()
{
    if (m_pWMemory != NULL && m_pWMemory->IsShown())
    {
        m_pWMemory->Show(false);
    }
    else
    {
        GetMemoryDialog();
...
void CEditorMainFrame::OnTimer( wxTimerEvent& event )
{
    ...
    wxApp::GetInstance()->ProcessIdle();//don't change the position. It's in order to handle the ExitEvent
...
{
    wxTreeItemId itemId = event.GetItem();
    wxString oldName = m_pComponentFileTC->GetItemText(itemId);
