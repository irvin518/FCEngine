...
BEGIN_EVENT_TABLE(CFCEditorComponentWindow, wxGLCanvas)
    ...
    EVT_KEY_DOWN(CFCEditorComponentWindow::OnKeyDown)
END_EVENT_TABLE()
...
CFCEditorComponentWindow::CFCEditorComponentWindow(wxWindow *parent,
                                                     ...
                                                     const wxString& name)
                                                     : CFCEditorGLWindow(parent, pContext, NULL, pos, size, style | wxFULL_REPAINT_ON_RESIZE, name)
                                                     , m_glRC(NULL)
                                                     , m_pCamera(NULL)
                                                     , m_iWidth(0)
                                                     , m_iHeight(0)
                                                     ...
                                                     , m_pClickedComponent(NULL)
{
    ...
    m_pAddDependencyMenu->Connect(wxEVT_COMMAND_MENU_SELECTED, wxMenuEventHandler(CFCEditorComponentWindow::OnDependencyMenuClicked), NULL, this);
}
...
CFCEditorComponentWindow::~CFCEditorComponentWindow()
{
    ...
    BEATS_SAFE_DELETE(m_pClickedComponent);
}
...
{
    FC_PERFORMDETECT_START(ePNT_ComponentWindow_Prepare)
    ...
    FC_PERFORMDETECT_STOP(ePNT_ComponentWindow_Prepare)

    FC_PERFORMDETECT_START(ePNT_ComponentWindow_Grid)
    RenderGridLine();
    FC_PERFORMDETECT_STOP(ePNT_ComponentWindow_Grid)

    FC_PERFORMDETECT_START(ePNT_ComponentWindow_DraggingLine)
    RenderDraggingDependencyLine();
    FC_PERFORMDETECT_STOP(ePNT_ComponentWindow_DraggingLine)

    CRenderGroupManager::GetInstance()->Clear();
    FC_PERFORMDETECT_START(ePNT_ComponentWindow_Components)
    RenderComponents();
    FC_PERFORMDETECT_STOP(ePNT_ComponentWindow_Components)

    pRenderer->LineWidth(1.0f);
    CRenderManager::GetInstance()->ContextFlush();
    FC_PERFORMDETECT_START(ePNT_ComponentWindow_RenderGroup)
    CRenderGroupManager::GetInstance()->Render();
    FC_PERFORMDETECT_STOP(ePNT_ComponentWindow_RenderGroup)

    SwapBuffers();
    //HACK: Restore the context, or the screen will flick, Don't know why.
    wglMakeCurrent(curHDC, m_glRC->GetGLRC());
}
...
{
    FC_PERFORMDETECT_START(ePNT_ComponentWindow_ComponnetInstance)
    ...
    FC_PERFORMDETECT_STOP(ePNT_ComponentWindow_ComponnetInstance)
        ...
        if (aeraRectType == eCART_Dependency)
        {
            ...
            for (size_t i = 0; i < derivedClassGuid.size(); ++i)
            {
                ...
                if (pBase != NULL)//If it is NULL, it must be an abstract class.
                {
                    ...
                    m_menuItemAndGuidMap.insert(std::map<wxMenuItem*, size_t>::value_type(pItem, derivedClassGuid[i]));
                }
...
void CFCEditorComponentWindow::OnDependencyMenuClicked(wxMenuEvent& event)
{
    ...
    CComponentEditorProxy* pProxy = static_cast<CComponentEditorProxy*>(CComponentProxyManager::GetInstance()->CreateComponent(iter->second, true));
...
void CFCEditorComponentWindow::ResetProjectionMode()
{
    ...
    m_pCamera->SetCenterOffset(centerOffset);
}
...
CCamera* CFCEditorComponentWindow::GetCamera() const
{
    return m_pCamera;
}

void CFCEditorComponentWindow::OnKeyDown( wxKeyEvent& event )
{
...
void CFCEditorComponentWindow::OnTimer( wxTimerEvent& /*event*/ )
{
    if (IsShownOnScreen())
    {
        FC_PERFORMDETECT_START(ePNT_RenderComponentWindow)
        Render();
        FC_PERFORMDETECT_STOP(ePNT_RenderComponentWindow)
    }
}
