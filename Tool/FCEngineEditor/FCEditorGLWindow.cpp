...
void CFCEditorGLWindow::OnTimer( wxTimerEvent& /*event*/ )
{
    if ( IsShownOnScreen())
    {
        FC_PERFORMDETECT_START(ePNT_RenderSceneWindow)
        SetCurrent(*m_glRC);
        ...
        FC_PERFORMDETECT_START(ePNT_EngineCenterUpdate)
        CEngineCenter::GetInstance()->Update();
        FC_PERFORMDETECT_STOP(ePNT_EngineCenterUpdate)

        FC_PERFORMDETECT_START(ePNT_PerformDetector)
        ...
        FC_PERFORMDETECT_STOP(ePNT_PerformDetector)

        FC_PERFORMDETECT_STOP(ePNT_RenderSceneWindow)
