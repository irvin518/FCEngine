...
#include "Render/BatchRenderGroup.h"
...
CComponentGraphic_GL::CComponentGraphic_GL()
    : m_pFont(NULL)
{
    memset(m_pFrags, 0, sizeof(m_pFrags));
...
void CComponentGraphic_GL::CreateMaterials()
{
    ...
    m_pAtlas = CResourceManager::GetInstance()->GetResource<CTextureAtlas>(_T("Component.xml"), false);
...
{
    FC_PERFORMDETECT_START(ePNT_ComponentWindow_InstanceHead)

    FC_PERFORMDETECT_START(ePNT_ComponentWindow_InstanceHead_1)
    CreateMaterials();
    FC_PERFORMDETECT_STOP(ePNT_ComponentWindow_InstanceHead_1)

    FC_PERFORMDETECT_START(ePNT_ComponentWindow_InstanceHead_2)
    ...
    head.tl.tex = m_pFrags[eCT_RectBG]->Quad().tl;
    ...
    head.bl.tex = m_pFrags[eCT_RectBG]->Quad().bl;
    ...
    head.tr.tex = m_pFrags[eCT_RectBG]->Quad().tr;
    ...
    head.br.tex = m_pFrags[eCT_RectBG]->Quad().br;
    CRenderBatch *batch = renderGroup->GetRenderBatch(m_pMaterial, GL_TRIANGLES, true);
    batch->AddQuad(head);

    FC_PERFORMDETECT_STOP(ePNT_ComponentWindow_InstanceHead_2)

    FC_PERFORMDETECT_START(ePNT_ComponentWindow_InstanceHead_3)

    // 2. Draw title text.
    m_pFont->RenderText(m_pOwner->GetDisplayName(), head.tl.position.x + Font_Render_X_Offset, head.tl.position.y, 0xFFFFFFFF);
    FC_PERFORMDETECT_STOP(ePNT_ComponentWindow_InstanceHead_3)

    FC_PERFORMDETECT_START(ePNT_ComponentWindow_InstanceHead_4)
    // 3. Draw Connect rect.
    ...
    FC_PERFORMDETECT_STOP(ePNT_ComponentWindow_InstanceHead_4)

    FC_PERFORMDETECT_START(ePNT_ComponentWindow_InstanceHead_5)

    batch = renderGroup->GetRenderBatch(m_pMaterial, GL_TRIANGLES, true);
    batch->AddQuad(connectRect);
    FC_PERFORMDETECT_STOP(ePNT_ComponentWindow_InstanceHead_5)

    FC_PERFORMDETECT_STOP(ePNT_ComponentWindow_InstanceHead)
}

void CComponentGraphic_GL::DrawDependencies( float cellSize )
{
    FC_PERFORMDETECT_START(ePNT_ComponentWindow_Dependency)
    CreateMaterials();
    ...
    FC_PERFORMDETECT_STOP(ePNT_ComponentWindow_Dependency)
}

void CComponentGraphic_GL::DrawDependencyLine( float /*cellSize*/, const CDependencyDescription* pDependency )
{
    FC_PERFORMDETECT_START(ePNT_ComponentWindow_DependencyLine)
    ...
    FC_PERFORMDETECT_STOP(ePNT_ComponentWindow_DependencyLine)
}

void CComponentGraphic_GL::DrawSelectedRect(float fCellSize)
{
    FC_PERFORMDETECT_START(ePNT_ComponentWindow_SelectRect)
    CreateMaterials();
    ...
    CRenderBatch *batch = renderGroup->GetRenderBatch(m_pMaterial, GL_TRIANGLES, true);
    batch->AddQuad(rect);
    FC_PERFORMDETECT_STOP(ePNT_ComponentWindow_SelectRect)
}
